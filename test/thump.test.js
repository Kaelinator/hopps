...
describe('thump.set', () => {
  ...
  it('throws TypeError when template is not a string nor an array', () => {
    ...
    const noParam = () => thump.set()

    expect(noParam).toThrow(TypeError)
    expect(noParam).toThrow('template must be of type string or array, recieved undefined.')

    expect(() => thump.set(42)).toThrow(/number/)
    expect(() => thump.set(null)).toThrow(/null/)
    expect(() => thump.set({})).toThrow(/object/)
    expect(() => thump.set(true)).toThrow(/boolean/)
  })
